...
{
  fprintf(stderr, "Usage: %s [-m memarray][-p percent][-q percent1][-v verbose][-s silent][-t tol][-d delta][-f trials][-a attempts] <infile> <vecor_file> <outfile>\n", argv0);
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "m:p:q:v:s:t:d:f:I:a:h")) != -1) {
    switch (opt) {
    case 'm':
      m1 = (long) atof(optarg);
      break;
  ...
  float *report = (float *) malloc((maxiter+3)*sizeof(float));
  
  ftime(&t1);
  iter = scale(m,i,j,x, z,b,report,1-silent,tol,perc,perc1,maxiter,delta,trials);
  ...
  while (iter < 0 && count++ < attempts) {
    ...
    for (p=0;p<n;p++) z[p] = z0[p];
    ftime(&t1);
    iter = scale(m,i,j,x, z,b,report,1-silent,-1,perc,perc1,maxiter,delta,trials);
    ftime(&end);
    printf("iterations took %15.10lf seconds\n",((float) (end.time - t1.time)) + 0.001*(end.millitm - t1.millitm) );
    if (silent) for (p=0;p<abs(iter);p++) printf("%d: %30.15lf\n",(int) (p+1),report[p]);
    printf("final error in scaling vector is %g and in row sums is %g\n",report[maxiter+1],report[maxiter+2]);
